...
class SingleTeacher:
    ...
    def _build_model(self, word_vectors):
        ...
        q2v_feats_fast, _ = cq_attention(vfeats, qfeats, mask1=v_mask, mask2=q_mask, drop_rate=self.drop_rate,
                                    reuse=False, name='q2v_attn_fast')
        v2q_feats_fast, _ = cq_attention(qfeats, vfeats, mask1=q_mask, mask2=v_mask, drop_rate=self.drop_rate,
                                    reuse=False, name='v2q_attn_fast')
        fuse_feats_fast = cq_concat(q2v_feats_fast, v2q_feats_fast, pool_mask=q_mask, reuse=False, name='cq_cat_fast')

        fast_start_logits, fast_end_logits = conditioned_predictor(fuse_feats_fast, dim=self.configs.model.dim, reuse=False, mask=v_mask,
