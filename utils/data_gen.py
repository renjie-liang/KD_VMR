...
import torch
def align_teacher_label(tlogits, vlen, max_vlen):
    vlen = min(vlen, max_vlen)
    # tlogits = torch.from_numpy(tlogits)
    tlogits = torch.nn.functional.interpolate(tlogits.unsqueeze(0), size=vlen, mode='linear', align_corners=True).squeeze(0)
    tlogits = torch.nn.functional.pad(tlogits, (0, max_vlen-tlogits.shape[1]), mode='constant', value=0)
    return tlogits.numpy()
...
def dataset_gen(data, vfeat_lens, word_dict, char_dict, configs):
    ...
    for record in tqdm(data, total=len(data), desc='Process data'):
        ...
        if vid not in vfeat_lens:
            ...
            continue
        vlen = vfeat_lens[vid]
        s_ind, e_ind, _ = time_to_index(record['stime'], record['etime'], vlen, record['duration'])
        word_ids, char_ids = [], []
        ...
        if "t0_logits" in record.keys():
            label_t0 = align_teacher_label(record["t0_logits"], vlen, max_vlen)
            label_t1 = align_teacher_label(record["t1_logits"], vlen, max_vlen)
            label_t2 = align_teacher_label(record["t2_logits"], vlen, max_vlen)
            result["label_t0"] = label_t0
...
def gen_or_load_dataset(configs):
    ...
    save_path = os.path.join(configs.paths.cache_dir, '_'.join([configs.task, str(configs.model.max_vlen), configs.model.name]) + '.pkl')
