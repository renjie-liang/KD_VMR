...
class TrainLoaderMultiTeacher:
    def __init__(self, dataset, visual_features, configs):
        super(TrainLoaderMultiTeacher, self).__init__()
        self.dataset = dataset
        self.visual_feats = visual_features
        self.batch_size = configs.train.batch_size
    ...
    def batch_iter(self):
        ...
        for index in range(0, len(self.dataset), self.batch_size):
            batch_data = self.dataset[index:(index + self.batch_size)]
            vfeats, vfeat_lens, word_ids, char_ids, s_labels, e_labels, h_labels,\
                  slabel_t0, slabel_t1, slabel_t2, elabel_t0, elabel_t1, elabel_t2 = self.process_batch(batch_data)
            yield batch_data, vfeats, vfeat_lens, word_ids, char_ids, s_labels, e_labels, h_labels,\
                  slabel_t0, slabel_t1, slabel_t2, elabel_t0, elabel_t1, elabel_t2

    def process_batch(self, batch_data):
        vfeats, word_ids, char_ids, s_inds, e_inds = [], [], [], [], []
        slabels_t0, slabels_t1, slabels_t2, elabels_t0, elabels_t1, elabels_t2 = [], [], [], [], [], []
        # max_vlen = 64
        for data in batch_data:
            vfeat = self.visual_feats[data['vid']]
            vfeats.append(vfeat)
            ...
            s_inds.append(data['s_ind'])
            e_inds.append(data['e_ind'])
            slabels_t0.append(data["label_t0"][0,:])
            slabels_t1.append(data["label_t1"][0,:])
            slabels_t2.append(data["label_t2"][0,:])
            elabels_t0.append(data["label_t0"][1,:])
            elabels_t1.append(data["label_t1"][1,:])
            elabels_t2.append(data["label_t2"][1,:])
        ...
        char_ids = np.asarray(char_ids, dtype=np.int32)  # (batch_size, w_seq_len, c_seq_len)
        # process video features
        # vfeats, vfeat_lens = pad_video_seq(vfeats, max_length=max_vlen)
        vfeats, vfeat_lens = pad_video_seq(vfeats)
        vfeats = np.asarray(vfeats, dtype=np.float32)  # (batch_size, v_seq_len, v_dim)
        vfeat_lens = np.asarray(vfeat_lens, dtype=np.int32)  # (batch_size, )
        # process labels
        max_vlen = np.max(vfeat_lens)
        s_labels = np.zeros(shape=[batch_size, max_vlen], dtype=np.float32)
        e_labels = np.zeros(shape=[batch_size, max_vlen], dtype=np.float32)
        match_labels = np.zeros(shape=[batch_size, max_vlen], dtype=np.int32)  # (batch_size, v_seq_len)
        new_s_labels, new_e_labels = [], []
        for idx in range(batch_size):
            st, et = s_inds[idx], e_inds[idx]
            cur_max_len = vfeat_lens[idx]
            ...
            match_labels[idx][(new_st_r + 1):new_et_l] = 2  # add I-M labels
            match_labels[idx][new_et_l:(new_et_r + 1)] = 3  # add E-M labels
            # Ssoft, Esoft, _ = gene_soft_label(st, et, cur_max_len, max_vlen, 0.3)
            # new_s_labels.append(Ssoft)
            # new_e_labels.append(Esoft)
        slabels_t0 = np.asarray(slabels_t0)[:,:max_vlen]
        slabels_t1 = np.asarray(slabels_t1)[:,:max_vlen]
        slabels_t2 = np.asarray(slabels_t2)[:,:max_vlen]
        elabels_t0 = np.asarray(elabels_t0)[:,:max_vlen]
        elabels_t1 = np.asarray(elabels_t1)[:,:max_vlen]
        elabels_t2 = np.asarray(elabels_t2)[:,:max_vlen]
        # print(elabels_t2.shape)
        return vfeats, vfeat_lens, word_ids, char_ids, s_labels, e_labels, match_labels,\
              slabels_t0, slabels_t1, slabels_t2, elabels_t0, elabels_t1, elabels_t2
